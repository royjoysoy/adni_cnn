...
def save_gradcam_visualization(inputs, cam, class_idx, save_path, epoch, batch_idx):
    try:
        ...
        plt.imshow(original_slice[:, :, original_slice.shape[2]//2], cmap='gray')
        plt.axis('off')
        ...
        cam_resized = zoom(cam, (atlas_data.shape[0] / cam.shape[0],
                                atlas_data.shape[1] / cam.shape[1],
                                atlas_data.shape[2] / cam.shape[2]), order=1)
        
        plt.imshow(cam_resized[:, :, cam_resized.shape[2]//2], cmap='jet', alpha=0.5)
        plt.axis('off')
