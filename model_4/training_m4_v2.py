...
from utils_m4 import calculate_f_score 
from utils_m4 import WarmupCosineScheduler, TrainingConfig 
...
def train_epoch(net, data_loader, optimizer, temperature, simclr_batch_size, aug_type, simclr_epoch, total_simclr_epochs, writer, scaler):
    ...
    try:
        ...
        effective_batch_size = first_batch[0].size(0)
        logging.info(f"Effective batch size: {effective_batch_size}")
    except Exception as e:
        logging.error(f"Error getting batch size: {str(e)}")
        effective_batch_size = simclr_batch_size
        logging.warning(f"Falling back to simclr_batch_size: {simclr_batch_size}")
    finally:
        accumulation_steps = max(1, simclr_batch_size // effective_batch_size)
        logging.info(f"Using gradient accumulation steps: {accumulation_steps}")
    ...
    for step, data in enumerate(train_bar):
        ...
        with torch.amp.autocast(device_type='cuda', enabled=True):
            ...
            sim = torch.mm(out, out.t().contiguous()) / temperature
            sim_max, _ = torch.max(sim, dim=1, keepdim=True)
            sim = sim - sim_max.detach()
            
            exp_sim = torch.exp(sim)
            mask = (torch.ones_like(exp_sim) - torch.eye(2 * pos_1.size(0), device=exp_sim.device)).bool()
            exp_sim = exp_sim.masked_select(mask).view(2 * pos_1.size(0), -1)
            
            pos_sim = torch.sum(out_1 * out_2, dim=-1) / temperature
            pos_sim = torch.cat([pos_sim, pos_sim], dim=0)
            pos_sim = torch.exp(pos_sim - sim_max.squeeze().detach())
            
            denominator = exp_sim.sum(dim=-1)
            loss = (-torch.log(pos_sim / (denominator + 1e-7))).mean() / accumulation_steps
        ...
        if not torch.isnan(loss):
            ...
            logging.warning(f"NaN loss detected in {aug_type} at epoch {simclr_epoch}, step {step}")
    ...
    if (len(train_bar) % accumulation_steps) != 0:
        scaler.unscale_(optimizer)
