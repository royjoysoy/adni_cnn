...
from utils_RS_oct import calculate_f_score
...
def run_ablation_study(net, train_loaders, val_loader, optimizer, scheduler, simclr_epochs, temperature, simclr_batch_size, k, save_top_n, writer, scaler):
    ...
    for simclr_epoch in range(1, simclr_epochs + 1):
        ...
        writer.add_scalar('Loss/avg_epoch', avg_epoch_loss, simclr_epoch)
        logging.info(f"Average SimCLR epoch loss: {avg_epoch_loss:.4f}")
        ...
        for aug_type in results:
            logging.info(f"  {aug_type} - Train Loss: {results[aug_type]['train_loss'][-1]:.4f}, Val Accuracy: {results[aug_type]['val_accuracy'][-1]:.2f}%")
