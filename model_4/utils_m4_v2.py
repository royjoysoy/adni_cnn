...
class TrainingConfig:
    ...
    def get_default_config(cls):
        ...
        DEFAULT_SAVE_DIR = '/home/simclr_project/simclr/SimCLR_RS/model_4/results_4_v1'
...
def plot_augmentation_matrix(results, save_path):
    """
    Create a matrix visualization of augmentation combination results
    """
    import matplotlib.pyplot as plt
    import seaborn as sns
    import numpy as np
    
    # Extract primary augmentation types
    primary_augs = ['geometric_intensity', 'noise_artifact', 'clinical_acquisition']
    
    # Create matrix
    n = len(primary_augs)
    matrix = np.zeros((n, n))
    
    # Fill diagonal with individual augmentation results
    for i, aug in enumerate(primary_augs):
        if aug in results:
            matrix[i, i] = results[aug]['best_accuracy']
    
    # Fill off-diagonal with combination results
    for i, aug1 in enumerate(primary_augs):
        for j, aug2 in enumerate(primary_augs):
            if i < j:
                combo_name = f"{aug1}_{aug2}"
                if combo_name in results:
                    matrix[i, j] = results[combo_name]['best_accuracy']
                    matrix[j, i] = results[combo_name]['best_accuracy']
    
    # Create visualization
    plt.figure(figsize=(10, 8))
    sns.heatmap(matrix,
                xticklabels=primary_augs,
                yticklabels=primary_augs,
                annot=True,
                fmt='.1f',
                cmap='YlOrRd')
    plt.title('Augmentation Combination Results (Accuracy %)')
    plt.tight_layout()
